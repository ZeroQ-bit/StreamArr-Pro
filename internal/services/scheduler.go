...
func InitializeDefaultServices() {
	...
	GlobalScheduler.Register(ServiceRDLibrarySync, "Adds cached library streams to Real-Debrid so Plex export has items to materialize", 15*time.Minute, true)
	GlobalScheduler.Register(ServicePlexExport, "Exports cached library items into Plex-visible filesystem paths", 15*time.Minute, true)
}
